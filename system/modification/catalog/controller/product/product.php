	...
	public function index() {
		...
		if ($product_info) {
			...
			if ($productaffilate['status'] == 1) {
				$this->document->addStyle('catalog/view/theme/default/stylesheet/productaffilate.css');
			}
			//end affilate
...
$data['thumb_fixed'] = $this->model_tool_image->resize($product_info['image'], $this->config->get('config_image_additional_width'), $this->config->get('config_image_additional_height'));
			...
			if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
				...
				if ($productaffilate['status'] == 1) {
					if ($productaffilate['switch_price_product'] == 0)
					{ $data['price'] = false; }
				}
			...
			if ($productaffilate['status'] == 1) {
				...
				$total_columns = count($data['pf_columns']);
				$column_width = (float)(100 / $total_columns);
				$data['column_width'] = number_format($column_width, 2, '.', '');
							...
							if(strtolower(trim($pf_column['title'])) != 'stores') {
								foreach($val as $key => $d) {
									if(strtolower(trim($key)) == strtolower(trim($pf_column['id']))) {
										...
										$data['affilates'][$k]['columns'][$pf_column['id']] = array( 'id'	=> $pf_column['id'],
																										   'type'	=> $pf_column['type'],
																										   'value'	=> $d,
																										   'url'	=> $img_url,
																										   'width' => $pf_column['width']);
