...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
		...
		$data['tab_general'] = $this->language->get('tab_general');
				...
				// Affilate
				$data['tab_affilate'] = $this->language->get('tab_affilate');
				$data['button_copy_affilate'] = $this->language->get('button_copy_affilate');
				$data['button_add_affilate'] = $this->language->get('button_add_affilate');
		...
		if (isset($this->request->post['product_option'])) {
			...
			$product_options = array();
				...
				$data['affiliate_module_install'] = false;
				$productaffilate = $this->config->get('productaffilate');
				if(isset($productaffilate['status']) && $productaffilate['status'] == 1) {
					$data['affiliate_module_install'] = true;
					...
					$pf_columns[] = array(	'id' => $product_affilates_columns_fix['store_ids']['id'],
											'title' => $product_affilates_columns_fix['store_ids']['title'],
											'type' => $product_affilates_columns_fix['store_ids']['type'],
											'sort_order' => $product_affilates_columns_fix['store_ids']['sort_order']
									);
					
					if(!empty($product_affilates_columns)) {
						foreach($product_affilates_columns as $product_affilates_column ) { 
							$pf_columns[] = array(
												'id'	=> $product_affilates_column['id'],
					...
					$col_span = sizeof($pf_columns);
					$data['col_span'] = $col_span;
					...
					if (isset($this->request->post['productaffilate_var'])) {
						$product_affilates = $this->request->post['productaffilate_var'];
					} elseif (isset($this->request->get['product_id'])) {
						$product_affilates = $this->model_catalog_productaffilate->getProductAffilates($this->request->get['product_id']);
						$pa_rows = array();
						...
						foreach ($product_affilates as $product_affilate) {
							...
							if(isset($pa_rows[$c][2])) {} else { $pa_rows[$c][2]  	= array(); }
						...
						$product_affilates  = array();
					}
	
					$k = 0;
					$data['product_affilates'] = array();
					foreach ($product_affilates as $title => $val) {
							$data['product_affilates'][$k] = array(
						...
						foreach($pf_columns as $pf_column) {
							...
							foreach($val as $key => $d) {
								if(strtolower(trim($key)) == strtolower(trim($pf_column['id']))) {
									$img_url = '';
									if($pf_column['type'] == 'image') {
										if (file_exists(DIR_IMAGE . $d)) {
											$img_url = $this->model_tool_image->resize($d, 70, 70);
										} else {
											$img_url = $this->model_tool_image->resize('no_image.jpg', 70, 70);
										}
									}
									$data['product_affilates'][$k]['columns'][$pf_column['id']] = array('id' => $pf_column['id'], 
																									'title'	=>$pf_column['title'], 																									'type'	=> $pf_column['type'], 																									'value'	=> $d, 
																									'url'	=> $img_url);
									$column_found = true;
								}
							}
							...
							if(!$column_found) {
								$no_image = '';
								if($pf_column['type'] == 'image') {
									$no_image = $this->model_tool_image->resize('no_image.jpg', 70, 70);
								} 
								$data['product_affilates'][$k]['columns'][$pf_column['id']] = array('id'=> $pf_column['id'],
																									'title'	=>$pf_column['title'],
																									'type'	=> $pf_column['type'],
																									'value'	=> '',
																									'url'	=> $no_image);
							}
